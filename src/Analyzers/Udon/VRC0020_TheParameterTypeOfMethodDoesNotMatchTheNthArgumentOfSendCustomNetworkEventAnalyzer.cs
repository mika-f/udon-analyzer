...
public class TheParameterTypeOfMethodDoesNotMatchTheNthArgumentOfSendCustomNetworkEventAnalyzer : BaseDiagnosticAnalyzer
{
    public override DiagnosticDescriptor SupportedDiagnostic => DiagnosticDescriptors.TheParameterTypeOfMethodDoesNotMatchTheNthArgumentOfSendCustomNetworkEvent;
    ...
    private void AnalyzeReceiverMembers(SyntaxNodeAnalysisContext context, INamedTypeSymbol? receiver, string target, SeparatedSyntaxList<ArgumentSyntax> argumentSyntaxList)
    {
        ...
        if (argumentSyntaxList.Count - 2 == 0)
            return; // No parameters, no need to check for argument types

        // check parameter types
        var arguments = argumentSyntaxList.Skip(2).Select(w => context.SemanticModel.GetTypeInfo(w.Expression).Type).ToList();
        var parameters = handler.Parameters.Select(w => w.Type).ToList();

        foreach (var argument in arguments.Select((w, i) => new { Symbol = w, Index = i }))
        {
            var parameter = parameters[argument.Index];
            if (parameter.Equals(argument.Symbol, SymbolEqualityComparer.Default))
                continue;

            DiagnosticHelper.ReportDiagnostic(context, SupportedDiagnostic, argumentSyntaxList[argument.Index + 2], parameter.ToDisplayString(), handler.ToDisplayString(), argument.Index + 3, argument.Symbol.ToDisplayString());
        }
    }
}