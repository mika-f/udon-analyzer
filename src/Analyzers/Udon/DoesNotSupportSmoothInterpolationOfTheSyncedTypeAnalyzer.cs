...
public class DoesNotSupportSmoothInterpolationOfTheSyncedTypeAnalyzer : BaseDiagnosticAnalyzer
{
    private const string UdonSyncedAttributeFullyQualifiedName = "UdonSharp.UdonSyncedAttribute";
    public override DiagnosticDescriptor SupportedDiagnostic => DiagnosticDescriptors.DoesNotSupportSmoothInterpolationOfTheSyncedType;
    ...
    private void AnalyzeFieldDeclaration(SyntaxNodeAnalysisContext context)
    {
        ...
        if (declaration.HasAttribute(UdonSyncedAttributeFullyQualifiedName, context.SemanticModel))
        {
            ...
            if (!val.HasValue || val.Value is not 3 /* Smooth */)
                return;
            ...
            if (SymbolDictionary.Instance.IsSymbolCanSmoothSync(symbol.Type))
                return;
