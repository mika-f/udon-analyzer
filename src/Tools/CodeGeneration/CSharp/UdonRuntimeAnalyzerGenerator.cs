...
public static class UdonRuntimeAnalyzerGenerator
{
    public static CompilationUnitSyntax CreateGeneratedAnalyzerCode(string name, string? runtimeMinVersion, string? runtimeMaxVersion)
    {
        var members = new List<MemberDeclarationSyntax>
        {
            NamespaceDeclaration(ParseName("NatsunekoLaboratory.UdonAnalyzer.Udon"))
                .WithMembers(
                    CreateSyntaxList<MemberDeclarationSyntax>(
                        ClassDeclaration(name, runtimeMinVersion, runtimeMaxVersion)
                    )
                )
        };
    ...
    private static ClassDeclarationSyntax ClassDeclaration(string name, string? runtimeMinVersion, string? runtimeMaxVersion)
    {
        var members = new List<MemberDeclarationSyntax>
        {
        ...
        return SyntaxFactory.ClassDeclaration(name)
                            ...
                            .WithAttributeLists(DescribeAttributes(runtimeMinVersion, runtimeMaxVersion))
                            .WithBaseList(BaseClasses())
                            .WithMembers(CreateSyntaxList(members));
    }

    private static SyntaxList<AttributeListSyntax> DescribeAttributes(string? minRuntimeVersion, string? maxRuntimeVersion)
    {
        var attributes = new List<AttributeSyntax>
        {
            DescribeDiagnosticAnalyzerAttribute(),
            DescribeRequireUdonVersionAttribute(minRuntimeVersion, maxRuntimeVersion),
            DescribeRequireUdonSharpVersionAttribute()
        };
    ...
    private static AttributeSyntax DescribeRequireUdonSharpVersionAttribute()
    {
        var args = new List<ExpressionSyntax>
        {
